...
class SerialDataApp:
    def __init__(self, root):
        ...
        # Create and place data display labels
    ...
    def update_data_display(self, data):
        ...
        for key, value in data.items():
            if key not in self.data_labels:
                label = ttk.Label(self.data_frame, text=f"{key}:")
                value_label = ttk.Label(self.data_frame, text=f"{value}")
                label.grid(row=len(self.data_labels), column=0, padx=5, pady=5, sticky=tk.W)
                value_label.grid(row=len(self.data_labels), column=1, padx=5, pady=5, sticky=tk.W)
